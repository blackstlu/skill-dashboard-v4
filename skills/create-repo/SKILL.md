---
name: create-repo
description: 用 GitHub CLI（gh）把目前的專案發佈到 GitHub——建立與專案資料夾同名的公開儲存庫、綁定為遠端 origin，並推送現有的提交。當使用者要求建立 GitHub 儲存庫、把專案發佈或推上 GitHub 時使用。
---

# Create Repo（發佈專案到 GitHub）

把使用者目前所在的專案資料夾發佈到 GitHub：在 GitHub 上建立同名的公開儲存庫（repository）、綁定成本機的遠端 `origin`，並把現有的提交推送上去。

## 前置檢查

執行前先確認三件事；任何一項不成立就停下來告訴使用者，不要自行補救：
...
- 不要主動建立或修改任何檔案（README、.gitignore 都不要自作主張加）。
